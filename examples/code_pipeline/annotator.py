...
limit = args.limit
input_data_path = args.input_data_path # Folder to the input data
output_data_path = args.output_data_path # Folder to the output data
download_dir = args.download_dir # "/iopsstor/scratch/cscs/rmachace/models"
...
def batch_inference(batch) -> Dict[str, list]:
    prompts = [[{"role": "system", "content": instruction}, 
                {"role": "user", "content": content if len(content) < truncation_size else content[:truncation_size]}] for content in batch["content"]]
