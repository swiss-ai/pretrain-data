...
class MegatronTokenizedFile:
    ...
    def __init__(
        ...
        self.bin_file = self.output_folder.open(
            f"{self.filename}.bin", mode="wb", block_size=upload_block_size
        )
    ...
    def close(self):
        ...
        self.idx_file = self.output_folder.open(
            f"{self.filename}.idx", mode="wb", block_size=self.upload_block_size
        )
        # Index Header
...
class MegatronDocumentTokenizer(PipelineStepWithTokenizer):
    ...
    def __init__(
        ...
        super().__init__(tokenizer_name_or_path, None, post_processor)
