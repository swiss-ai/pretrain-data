...
class SlurmPipelineNodeExecutor(PipelineExecutor):
    ...
    Adapted from https://github.com/huggingface/datatrove/blob/slurm_nodes/src/datatrove/executor/slurm_nodes.py
    ...
    def launch_merge_stats(self):
        ...
        launch_slurm_job(
            self.get_launch_file_contents(
                ...
                f'srun {srun_args_str} merge_stats {self.logging_dir.resolve_paths("stats")} '
    ...
    def launch_job(self):
        ...
        extra_command = f"{self.extra_command};" if self.extra_command else ""
        launch_file_contents = self.get_launch_file_contents(
            self.get_sbatch_args(max_array),
            f"srun {srun_args_str} --kill-on-bad-exit=0 -l bash -c \"{extra_command} launch_pickled_pipeline {self.logging_dir.resolve_paths('executor.pik')}\"",
        )
        # save it
